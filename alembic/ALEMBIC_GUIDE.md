...
**⚠️ IMPORTANTE:** Siempre revisa el código generado. Alembic es inteligente pero no perfecto.
...
# ⚠️ Solo si sabes lo que haces (migración manual)
...
**Objetivo:** Índice en `audit_logs.timestamp` para queries más rápidas
...
graph TD
    ...
    C --> D{¿Correcto?}
    D -->|No| E[Editar migración]
    E --> F[alembic upgrade head]
    D -->|Sí| F
    F --> G{¿Funciona?}
    G -->|No| H[alembic downgrade -1]
    H --> E
    G -->|Sí| I[Probar downgrade]
    I --> J[alembic downgrade -1]
    J --> K{¿Funciona?}
    K -->|No| E
    K -->|Sí| L[alembic upgrade head]
...
graph LR
    ...
    E -->|Sí| G[Click: Aplicar Migraciones]
    G --> H[Verificar logs]
    H --> I{¿Éxito?}
